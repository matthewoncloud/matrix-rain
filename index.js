...
const runes = 'ᚠᚢᚦᚨᚱᚲᚷᚹᚺᚾᛁᛃᛇᛈᛉᛋᛏᛒᛖᛗᛚᛜᛞ';
...
rainDropText2dArray = Array.from({ length: columns }, () => Array.from({ length: Math.floor(canvas.height / fontSize) }, () => alphabet.charAt(Math.floor(Math.random() * alphabet.length))));
...
const draw = () => {
    ...
    context.fillStyle = 'rgba(0, 0, 0, 0.05)'; // Original: 0.05
    ...
    for(let i = 0; i < rainDrops.length; i++)
    {
        ...
        // leading character is drawn white so it stands out before fading to green
        context.fillStyle = '#FFF';
        context.fillText(text, i*fontSize, rainDrops[i]*fontSize);

        // repaint the previous row in green so the old head turns matrix green instead of staying white
        // context.fillStyle = 'rgba(255, 255, 255, 1)';
        // const text = alphabet.charAt(Math.floor(Math.random() * alphabet.length));
        // context.fillText(text, i*fontSize, (rainDrops[i])*fontSize);

        rainDropText2dArray[i][rainDrops[i]] = text;

        // context.fillStyle = '#0F0';
        // context.fillText(rainDropText2dArray[i][rainDrops[i]-1], i*fontSize, (rainDrops[i]-1)*fontSize);
        // context.fillStyle = 'rgba(0, 255, 0, 0.46)';
        // context.fillText(rainDropText2dArray[i][rainDrops[i]-2], i*fontSize, (rainDrops[i]-2)*fontSize);
        // context.fillStyle = 'rgba(0, 255, 0, 0.34)';
        // context.fillText(rainDropText2dArray[i][rainDrops[i]-3], i*fontSize, (rainDrops[i]-3)*fontSize);
        // context.fillStyle = 'rgb(0, 0, 0)';
        // context.fillText(rainDropText2dArray[i][rainDrops[i]-4], i*fontSize, (rainDrops[i]-4)*fontSize);
...
// console.log(rainDropText2dArray);